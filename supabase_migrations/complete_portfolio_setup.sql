...
-- =============================================
-- Description: Complete setup for portfolio system (safe to run multiple times)
-- Author: Fauzi M. Noor
-- Created: 2025-12-05
-- Version: 1.2 (Complete & Safe)
-- =============================================
...
BEGIN
  ...
  -- Count storage policies (use pg_policies for storage.objects)
  SELECT COUNT(*) INTO stor_pol_count
  FROM pg_policies
  ...
  RAISE NOTICE '  • Table columns: % (expected: 12)', col_count;
  RAISE NOTICE '  • Indexes: % (expected: 5+)', idx_count;
  RAISE NOTICE '  • RLS policies: % (expected: 5)', pol_count;
  RAISE NOTICE '  • Storage policies: % (expected: 4)', stor_pol_count;
  RAISE NOTICE '';

  IF col_count >= 12 AND idx_count >= 5 AND pol_count = 5 AND stor_pol_count = 4 THEN
    RAISE NOTICE '🎉 All components verified successfully!';
  ELSE
    RAISE NOTICE '⚠️  Some components may be missing. Please review.';
  END IF;
